...
No bound overloads (text(State), withClass(State), ...): they smuggle non-child references into params. Targeted updates come only from component scopes plus server diff — never from binding APIs. One static home per family: j2html.TagCreator tags, j2act.Routes route DSL, j2act.States reactive primitives, j2act.ui.Ui plus Variant styled tags. Events are typed after Vaadin Flow (.onClick(ClickEvent), .onChange with e.value(), file inputs with e.files()). with* component props live on a ComponentTag base with per-call-site memoized instances.

State placement: state() may be called in any render fn — field or method-local, memoized per call site (unconditional, hooks-like). Props flow explicitly: factories take children only, all props (data, State handles) arrive via with* builders. Reads of enclosing state inside tag-building fns are for small private helpers only.

Instance identity: each call site gets its own memoized component instance with its own State and its own stable scope anchor, so two counter() siblings never share state or morph targets. Same component repeated in each() needs an explicit withKey for stable identity across reorder/filter. Never hardcode withId inside a reusable component — derive ids from props/key or use withKey.
