...
Java 11 is the baseline because a large share of enterprise shops, including new apps, are stuck there. Java 8 is out because Oracle dropped it. Without virtual threads, blocking user code (JDBC, REST) must never run on socket I/O threads: every event, query loader, mutation and effect runs on a session's serial lane over a bounded pool sized for blocking work.
...
No framework dev tooling. The dev loop is the host's: run WildFly or Spring Boot under the IDE debugger and JVM HotSwap (Eclipse, IntelliJ, VS Code) swaps method bodies live. Next event or navigation re-renders with the new code. Structural changes need a redeploy, which kills sessions, and the client re-mounts per ADR 0010. Call-site memoization may re-mount instances whose call sites moved. That's acceptable. React-style HMR waits for the devtools work.
