...
Handlers cannot return bytes over a morph, and every app needs "Export CSV". download() is the mirror of upload(): a Mutation flavor whose body writes to an OutputStream, e.g. download(out -> csv.write(users, out)).withFileName("users.csv").withContentType("text/csv"), triggered with mutate() from any event.
