...
The token lives for a configurable TTL (withDownloadTtl, default 60s) and fails the run if the browser never fetches it. A fetch whose resolved identity differs from the session's is refused. The client triggers the fetch with a hidden download link, so the page and its socket stay put. withRetry is refused on a download: the browser runs the fetch, so a retry is another mutate().
